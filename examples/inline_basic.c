...
int add_hook(int a, int b) {
  ...
  add_func original = (add_func)snare_inline_get_trampoline(hook);
...
int main(void) {
  ...
  hook = snare_inline_new((void *)add, (void *)add_hook);
  if (!hook) {
    printf("hook creation failed\n");
    return 1;
  }
  printf("hook created, trampoline=%p\n", snare_inline_get_trampoline(hook));
